...
Deno.test("List.enumerate", () => {
  ...
  assertEquals([...result], [[0, 1], [1, 2], [2, 3]]);
});
...
Deno.test("List.groupBy", () => {
  const list = List.of({ type: "apple" }, { type: "banana" }, {
    type: "apple",
  });
...
Deno.test("List.random should return each element with roughly equal probability", () => {
  const list = List.from([1, 2, 3]);
...
Deno.test("List.updateAt", () => {
  const list = List.of({ name: "Alice" }, { name: "Bob" }, {
    name: "Charlie",
  });
